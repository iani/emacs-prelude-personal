...
* COMMENT use this emacs-lisp block to add memoirbook class to latex exporter
...
#+RESULTS:
| memoirbook | \documentclass[10pt]{memoir}  | (\chapter{%s} . \chapter*{%s}) | (\section{%s} . \section*{%s})       | (\subsection{%s} . \subsection*{%s})       | (\subsubsection{%s} . \subsubsection*{%s}) | (\paragraph{%s} . \paragraph*{%s})         | (\subparagraph{%s} . \subparagraph*{%s}) |
